...
try {
    ...
    if ($Command -eq '.setup') {
        ...
        if ($bun.status -ne 'ready' -or $usesLocalBun) {
            ...
            $guide = "Development-only Bun for this checkout. Source and hashes: skills/gidd/assets/runtimes.json and tools/bun/install.json.`nRemove .dev/ only when no setup or tests are running. External PATH tools and user-level GIDD tools are not owned here.`n"
