...
先在 .cache/<name>/payload 完成下载、官方 SHA-256、受控解压、版本与兼容检查，再把旧目录移动到 .cache/previous-<name>，发布新目录及启动器。失败时恢复旧目录；启动器发布成功后才清理备份。强制中断留下的备份由下次 bootstrap --yes 在锁内恢复，再重新检查；只读 bootstrap 不执行恢复。未知备份或恢复目标损坏时保留并报告冲突。Windows 文件占用可能使替换失败，不能承诺正在使用的运行时都能热替换。

启动器发布是提交点；之后的缓存清理失败以 cleanup_pending=true 报告，不回滚已发布安装。旧备份清理前先更名为 .cache/retired-<name>-<UUID>，避免部分清理的目录被当作可恢复备份；被运行进程占用的残留可在退出后清理。
