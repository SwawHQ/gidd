...
仓库内 `.agents/skills/gidd/` 的入口从自身位置自动定位目标（含 worktree），不依赖工作目录。主机与预期账号只读取仓库配置中的 `github.hostname` 和 `github.account`；缺失即报错，不临时覆盖或推断账号。`github.remote` 不参与授权，配置设置见 [configuration.md](configuration.md)。入口经 [bootstrap](bootstrap.md) 已发布的共享启动器直接执行 JS，再读取共享 gh 路径绑定；不搜索 PATH 或执行额外版本探测。启动器缺失时提示 bootstrap，不自动准备运行时。缺失 gh 时由 Agent 根据用户授权使用 [工具准备入口](setup.md)。申请授权要求 gh 2.98.0 或更新版本，由 bootstrap 验证并记录；原路径被外部替换可能需 doctor 检出。
