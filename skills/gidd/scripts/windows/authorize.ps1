[CmdletBinding()]
param([string]$RepositoryPath, [string]$Account, [string]$Hostname = 'github.com')
Set-StrictMode -Version Latest
$ErrorActionPreference = 'Stop'
[Console]::OutputEncoding = New-Object Text.UTF8Encoding($false)
try {
    if (-not $Account) { throw 'expected_account_required' }
    ...
    & $runtime.details.path (Join-Path $PSScriptRoot '../auth.mjs') --repository $target --hostname $Hostname --account $Account --gh $gh.details.path
    exit $LASTEXITCODE
} catch {
