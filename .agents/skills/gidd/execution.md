...
`spec.list --lang en|zh` returns `scope` and localized `name`/`description` entries from `specs/<name>/prompt.en.md` and `prompt.zh-CN.md`. Names may contain dot-separated lowercase segments with an optional two-digit prefix. The prefix is part of the exact selection name; presets use 00–13 and sort by full name. Underscore-prefixed helper directories and directories with neither prompt are skipped. Once either prompt exists, both front matter blocks must be valid. The YAML front matter requires `description` and optionally declares `issue_template` relative to that prompt. There is no filename fallback; omitted means no template. Both languages must agree on template presence and form structure. `spec <name>` and `spec.current` print expanded Markdown without metadata; `spec.issue <name>` and `spec.issue.current` return the referenced JSON form. Common Markdown is optional. After includes, the literal @gidd.link spec.issue.current@ placeholder becomes a template command bound to the printed spec and language; no command is executed. The five command names are fixed; `current` is not a reserved spec name. See [spec resources](references/specs.md) for resource authoring.
...
`spec.list` 从 `specs/<名称>/prompt.en.md` 和 `prompt.zh-CN.md` 的 YAML front matter 读取双语简介，返回 `scope` 和 `specs`。名称支持小写点分段及可选的两位数字前缀；编号是完整名称的一部分，预设使用 00–13 并按完整名称排序；下划线前缀辅助目录和没有任何 prompt 的占位目录不列入。有任意一种语言的 prompt 后，两种语言元数据均需有效。`description` 必填；`issue_template` 可选，路径相对于入口 Markdown，缺失表示未提供模板，无同名文件回退。双语模板声明及字段结构须一致。`spec <名称>`、`spec.current` 输出去除元数据并展开引用的正文；`spec.issue <名称>`、`spec.issue.current` 输出 JSON 模板。common Markdown 为可选引用；展开后，将 @gidd.link spec.issue.current@ 替换为绑定本次规范名称及语言的模板命令文字，不执行命令。命令名固定，`current` 无需作为规范保留名。资源编写见 [specs.md](references/specs.md)。
