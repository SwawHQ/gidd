...
Use the installed skill's gidd.cmd on Windows. Bare `gidd.cmd tools` shows help without a runtime. Use `tools --check` for read-only inspection; when preparation is authorized use `tools --ensure` to prepare a runtime, Git, gh and shared bindings. Check and ensure are exclusive. Old bootstrap/setup commands are retired. Read [references/bootstrap.md](references/bootstrap.md) for selection, force and recovery.
