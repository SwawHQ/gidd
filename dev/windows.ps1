...
$codeRoot = Join-Path $repoRoot '.agents/skills/gidd/scripts.powershell'
$toolsRoot = $null
$storage = $null
$lock = $null
...
try {
    ...
    if ($Command -eq '.setup') {
        ...
        if ($needsLocal) {
            ...
            $manifest = [IO.File]::ReadAllText((Join-Path $repoRoot '.agents/skills/gidd/scripts.powershell/runtimes.json')) | ConvertFrom-Json
