...
function Get-GiddBoundRuntime {
    param([string]$Root, [ValidateSet('','bun','node')][string]$Runtime = '')
    if (-not $Runtime) {
        foreach ($name in @('bun','node')) { $candidate = Get-GiddBoundRuntime $Root -Runtime $name; if ($candidate) { return $candidate } }
        return $null
    }
    try {
        $launcher = Join-Path $Root "$Runtime.link.cmd"
        ...
        if ($name -ne $Runtime) { return $null }
...
function Find-GiddBootstrapRuntime {
    ...
    $bound = Get-GiddBoundRuntime $Storage.tools_root -Runtime $Runtime
...
function Get-GiddLauncherExecutable {
    param($Candidate)
    $name = $Candidate.id.Substring(5)
    $binding = Get-GiddRuntimeBinding $Candidate
    $executable = if ($Candidate.details.source -eq 'managed') { "%~dp0$name\" + (Get-GiddExecutableName $name).Replace('/','\') }
...
function Get-GiddLauncherText {
    param($Candidate, $GitCandidate = $null)
    $executable = Get-GiddLauncherExecutable $Candidate
    ...
    return (@(
        ...
        if ($Candidate.id -eq 'tool.gh' -and $GitCandidate) {
            $gitExecutable = Get-GiddLauncherExecutable $GitCandidate
            ('set "PATH=' + $gitExecutable.Substring(0, $gitExecutable.LastIndexOf('\')) + ';%PATH%"')
            'set "GIT_EXEC_PATH="'
        }
...
function Restore-GiddInterruptedRuntime {
    ...
    $launcher = Join-Path $Root "$Name.link.cmd"
    Assert-GiddPlainPath $launcher
    $candidate = @{ id="tool.$Name"; details=@{ source='managed' } }
    $committedTexts = @((Get-GiddLauncherText $candidate))
...
function Invoke-GiddBootstrap {
    ...
    $root = $Storage.tools_root
    if ($Node) { $Runtime = 'node' }
    if ($Reinstall -and -not $Runtime) {
        $bound = Get-GiddBoundRuntime $root
        $Runtime = if ($bound) { $bound.id.Substring(5) } else { 'bun' }
    }
    if (-not $Yes) {
        $found = Find-GiddBootstrapRuntime $Storage -Runtime $Runtime
        $name = if ($found.candidate) { $found.candidate.id.Substring(5) } elseif ($Runtime) { $Runtime } else { 'bun' }
        $launcher = Join-Path $root "$name.link.cmd"
    ...
    try {
        foreach ($name in $(if ($Runtime) { @($Runtime) } else { @('bun','node') })) { Restore-GiddInterruptedRuntime $root $name }
        $found = Find-GiddBootstrapRuntime $Storage -Runtime $Runtime
        ...
        $name = $candidate.id.Substring(5)
        $launcher = Join-Path $root "$name.link.cmd"
        $action = Write-GiddLauncher $root (Get-GiddLauncherText $candidate) $name
