...
- tool-bindings.json: generated Git/gh absolute paths and last verified versions;
  ...
  scan PATH or rehash payloads. Rerun gidd.pre.ensure --repo <repository-path> when bindings/tools change.
...
Run the actual skill's gidd.pre.ensure.cmd --repo <repository-path> to prepare/repair
tools, bindings and the repository entry. The target must be a Git working-tree root
with a suitable GitHub remote. Add --check for read-only diagnosis or --jsruntime=node
to select Node. --force explicitly reinstalls the selected managed runtime, Git and gh;
neither selector nor force can accompany --check. Normal preparation reuses qualifying tools.
...
ones are moved. Failed stages keep other completed tools; rerun gidd.pre.ensure --repo <repository-path>.
