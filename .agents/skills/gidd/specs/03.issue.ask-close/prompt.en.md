---
description: "An Issue is required. Ask whether to close the Issue; the Agent continues other stages automatically."
...
## commit
Current policy: autonomous handling (commit includes push by default). However, if this would automatically close an Issue, obtain the user's authorization for that closure; otherwise, remove the automatic closure association.
...
## Merge
Current policy: if using a separate branch or PR workflow, you may merge autonomously once acceptance criteria and repository merge requirements are satisfied. However, if this would automatically close an Issue, obtain the user's authorization for that closure; otherwise, remove the automatic closure association.
