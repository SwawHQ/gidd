...
- tool-bindings.json: generated Git/gh absolute paths and last verified versions;
  ...
  scan PATH or rehash payloads. Rerun bootstrap when bindings/tools change.
...
Run gidd.cmd bootstrap to prepare/repair tools and bindings. bootstrap --check is
read only; bootstrap --node selects Node. --reinstall explicitly reinstalls the
selected managed runtime. Existing qualifying tools are reused, not upgraded.
...
ones are moved. Failed stages keep other completed tools; rerun bootstrap.
