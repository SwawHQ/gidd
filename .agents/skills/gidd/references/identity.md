...
入口经 [bootstrap](bootstrap.md) 已发布的共享启动器直接执行 JS，不调用 PowerShell。scripts/gidd.mjs 用共用 JS 探测 gh、Git，再调用 scripts/github.mjs。缺失 gh 不自动安装；启动器缺失时提示 bootstrap --yes，身份检查尚未执行。
