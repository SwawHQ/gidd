...
An entry at <repository>/.agents/skills/gidd/ locates its repository using its own path and local .git marker, including worktrees, independently of the current directory. Resolve the target repository before invoking scripts. Only this layout with a .git marker supplies an implicit target; client-managed user-level installations require an explicit target. Without an implicit target, doctor still reports tools and target_required; resolve the target from the user request before repository diagnosis, asking only when unclear. Do not infer it from a user skill directory or the calling directory. Installing the skill or preparing a launcher does not enable any repository.
...
Use [references/doctor.md](references/doctor.md) for offline diagnosis. Doctor checks Git/gh availability, the worktree, GitHub configuration fields and the selected remote against the configured hostname using local data. Missing Git, a non-Git directory, missing configuration and a remote mismatch are distinct results; none automatically starts initialization. Doctor does not install, log in or write configuration. Its runtime item describes the current process, not a new compatibility check or runtime selection. Missing objects and not_checked results must not be called passed checks or failed login. local_ready does not establish enablement, identity or push access.
