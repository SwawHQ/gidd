...
Current policy: once acceptance criteria and repository merge requirements are satisfied, you may autonomously merge the PR. However, if this would automatically close an Issue, obtain the user's authorization for that closure; otherwise, remove the automatic closure association.
