...
不应自动继续：表示仅报告结果，如何进一步行动询问用户授权（除非已有明确授意）。
...
## commit
当前规范：自动处置（commit 默认包含 push）——但若会自动关闭 Issue 的，应取得用户授权，否则解除自动关闭。
注意划分批次，避免大而混杂；而跨 Issue 的更是必须分开。
默认使用 Refs #NNN 关联 Issue。

## 合并
推送开发分支后，创建或更新指向目标分支的 PR。
当前规范：满足验收及仓库合并要求后，不应自动继续合并——会自动关闭 Issue 的，还需一并取得用户授意，否则解除自动关闭。
若发现：仓库Action/规范不通过、目标分支最终未收到交付等，如何处置，不应自动继续。
默认使用 Refs #NNN 关联 Issue。
