...
for /f "delims=" %%R in ('@ "%SystemRoot%\System32\WindowsPowerShell\v1.0\powershell.exe" -NoProfile -NonInteractive -ExecutionPolicy Bypass -File "%~dp0scripts\dev\windows.ps1" .runtime-path %GIDD_DEV_NAME% %GIDD_DEV_SOURCE%') do set "GIDD_DEV_RUNTIME=%%R"
"%SystemRoot%\System32\chcp.com" %GIDD_DEV_CODEPAGE% >nul
if not defined GIDD_DEV_RUNTIME exit /b 1
"%GIDD_DEV_RUNTIME%" "%~dp0scripts\dev\runtime.mjs" %*
exit /b %ERRORLEVEL%
